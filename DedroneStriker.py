...
def connectVehicle(event):
    global app, master, ip, boot_time, takeoff_height, guided_mode, takeoff_active, ignoreVehicleThresh, triggerRelayThresh, control_thread, flask_thread, shutdownFlag
    shutdownFlag = False
    ip = get_ip_address()
    ipAddr_var.set("udp:"+ip+":14553")
    # Initialize global variables for drone control
    master = mavutil.mavlink_connection("udp:"+ip+":14553", source_system=1) # UDP Connection Forwarded from GCS
...
def stopConnection(event):
    global shutdownFlag
    shutdownFlag = True
    control_thread.join()
    print("Control thread closed")
    requests.post('http://'+ip+':8080/shutdown')
    master.close()
        ...
        if 'detections' in data and data['detections']:
            ...
            if latest_detection['identification']['detectionType'] == 'drone':    
                ...
                if shutdownFlag:
                    ...
                    lat_var.set(target_lat)
                    long_var.set(target_lon)
                    alt_var.set(target_alt)
...
if __name__ == '__main__':
    global ipAddr_var, lat_var, long_var, alt_var

    ##### Main Program #####
    gui = Tk()

    # Set the title of the window
    gui.title("Dedrone Striker")
    ...
    #Stop Button
    connectButton = Button(gui, text="Stop")
    connectButton.grid(row=4, column=1)
    connectButton.bind('<ButtonPress-1>',stopConnection)
